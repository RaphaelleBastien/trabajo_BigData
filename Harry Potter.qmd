...
Por desgracia, no podré hablar de todo lo que quisiera, como las criaturas mágicas o los lugares importantes del universo de Harry Potter. Pero intentaré darte toda la información divertida posible.
...
# Y JK? 
library(readxl)
author <- read_excel("author.xlsx", col_types = c("text", 
    "numeric"))
author <-ggplot(author, aes(x= Author ,y= revenues_in_millions_of_dollars , fill= Author)) +
  labs(title = "Gráfico 5 : Los ingresos de algunos autores en los últimos años ",
      caption  = "Fuente: Statista")+
  geom_bar(stat = "identity")+ 
  theme_light()+
  theme (axis.text.x = element_text(angle = 90,
                                   hjust = 1,
...
### D. Las Pociones y sus efectos ![](imagenes/Pociones.png){width="30"}
...
JK Rowling lo pasó mal al principio. Ninguna editorial quiso publicar este libro infantil. Pero finalmente lo hizo y el primer volumen se convirtió rápidamente en un éxito de ventas. Hoy se han vendido más de 522 millones de ejemplares en todos los volúmenes. El pequeño mago huérfano se convirtió rápidamente en el mago más popular de su generación. Muchos lectores han crecido con él, lo que lo hace aún más notable, creo yo.
...
7 volúmenes y 8 películas. Podemos afirmar que la franquicia Harry Potter ha sido todo un éxito, y lo vemos claramente en los resultados de taquilla. En algunos estudios, Harry Potter y las Reliquias de la Muerte figura entre las 50 mejores películas de las últimas décadas.El pequeño mago huérfano se convirtió rápidamente en el mago más popular de su generación. También es importante saber que para 8 películas hubo 5 directores diferentes, cada uno aportando su toque personal, haciendo que cada película sea un poco más mágica que la otra.

¿Lo sabías? El actor Richard Harris, que interpretó al director Albus Percival Wulfric Brian Dumbledore en las dos primeras películas, lamentablemente falleció antes de rodar la secuela... por lo que Michael Gambon le sustituirá durante el resto de las películas. ¿Has notado la diferencia? Personalmente, no.

![Carteles de cine](imagenes/harry-potter-movies.png)

# 5. El mercado creado por el universo Harry Potter

### A. Warner Bros Studio en Londres

Por supuesto, se ha creado toda una franquicia de productos en torno a este universo. Había nacido el Wizarding World, que reúne la franquicia cinematográfica "Fantastic Beasts". Hoy es posible visitar los estudios de Harry Potter en Londres, que son muy populares. Allí se encuentran los decorados, el vestuario y los animales que aparecen en la saga. Todo un sueño para los amantes de Harry Potter.

Por desgracia, aún no he tenido la oportunidad de ir, así que no puedo dar mi opinión al respecto. Pero en el gráfico (no cuento 2020...) se puede ver que cuando se estrenó la segunda película de la franquicia "Animales fantásticos", ¡el número de visitantes también aumentó significativamente!

```{r}
#| code-fold: true
warner <- ggplot(Warner_Bros_studio, aes(x = Year, y = revenue_in_thousands_of_pounds )) + 
  labs(title = "Gráfico 6: Ingresos anuales por visitas a los estudios de Warner Bros. en Londres de 2012 a 2021 " ,
       caption  = "Fuente: Statitas") +
  geom_line()
...
Pero no todo ocurre en Londres. Puedo decirte que el Wizarding World de Universal Studio (Orlando, EE.UU.) bien merece una visita. Además de las emocionantes atracciones, los escenarios son sublimes. ¡Por cierto, recomiendo la butterbeer, no tiene alcohol solo un delicioso sabor a caramelo!

Para que te entren ganas de ir al Estudio, aquí tienes una foto, espero que te convenza

![](imagenes/The-Wizarding-World-of-Harry-Potter-Diagon-Alley.jpg){width="574"}

Sí, sí, el dragón respira fuego... ¡cuidado!

# Bibliografía

La mayoría de mis datos están en:

Kaggle: <https://www.kaggle.com/search?q=harry+potter+dataset>

Statista: <https://es.statista.com/temas/3985/la-saga-de-harry-potter/>

Para algunos de los datos tuve que buscar varios artículos en internet, ¡fue bastante largo!
